...
def main(argv=None):
    ...
    for file in files:
        ...
        metadata, profiles, bad_file = read_product_file(file)
        ...
        if not bad_file:
            ...
            if layer_obj.layers is not None:
            ...
            if ltool_settings["store_layer_object"]:    
                list_obj.append(layer_obj)
                
            if ltool_settings["report_file_status"]:
                list_good.append(file)
        
        else:
            if ltool_settings["report_file_status"]:
                list_bad.append(file)
    
    list_geom = list_geom[0] if len(list_geom) == 1 else list_geom
    list_obj = list_obj[0] if len(list_obj) == 1 else list_obj
    
    return list_geom, list_obj, list_good, list_bad        
