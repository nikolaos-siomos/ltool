...
def plot_layers(dir_out, fname, ms_id, layers, height, 
    ...
    if valid_range.any():
        ...
        max_available_height = np.max(height[sig==sig])
        
        if max_height < max_available_height:
            max_height = round(max_available_height + 0.5)
        ...
        if not np.isnan(wct).all():
            plt.fill_betweenx(height, -snr_factor * wct_err, 
                              snr_factor * wct_err,
                              alpha = 0.3, color = 'tab:blue')
